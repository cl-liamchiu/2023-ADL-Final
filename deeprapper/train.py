...
def main():
    ...
    if args.bpe_token:
        ...
        full_tokenizer.add_special_tokens({"additional_special_tokens": ["[BEAT]", "[S]", "[M]", "[F]"]})
        
        full_finalizer = BertTokenizer(
            vocab_file=args.finalizer_path,
            tokenize_chinese_chars=False,
            do_lower_case=False
        )
        full_finalizer.add_special_tokens({"additional_special_tokens": ["[BEAT]", "[S]", "[M]", "[F]"]})
        
        full_sentencer = BertTokenizer(
            vocab_file=args.sentencer_path,
            tokenize_chinese_chars=False,
            do_lower_case=False
        )

        full_poser = BertTokenizer(
            vocab_file=args.poser_path,
            tokenize_chinese_chars=False,
            do_lower_case=False
        )

        full_beater = BertTokenizer(
            vocab_file=args.beater_path,
            tokenize_chinese_chars=False,
            do_lower_case=False
        )
