...
def main(first_line, model):
    ...
    parser.add_argument('--temperature', default=1.0, type=float, required=False, help='sampling temperature')
    ...
    parser.add_argument('--topp', default=0.0, type=float, required=False, help='sample from topp tokens')
    ...
    config.read('generate_config.ini') 

    
    for key in config['DEFAULT']:
        if hasattr(args, key):
            if type(getattr(args, key)) == float:
                setattr(args, key, float(config['DEFAULT'][key]))
            elif type(getattr(args, key)) == int:
                setattr(args, key, int(config['DEFAULT'][key]))
            elif type(getattr(args, key)) == bool:
                setattr(args, key, config['DEFAULT'].getboolean(key))
            else:
                setattr(args, key, config['DEFAULT'][key])

    args.prefix = first_line
    print('args:\n' + args.__repr__())
    ...
    tokenizer = BertTokenizer(vocab_file=args.tokenizer_path, do_lower_case=False)
    tokenizer.add_special_tokens({"additional_special_tokens": ["[BEAT]", "[S]", "[M]", "[F]"]})
    ...
    if args.enable_final:
        ...
        finalizer = BertTokenizer(vocab_file=args.finalizer_path, tokenize_chinese_chars=False, do_lower_case=False)
        tokenizer.add_special_tokens({"additional_special_tokens": ["[BEAT]", "[S]", "[M]", "[F]"]})
    ...
    if args.enable_sentence:
        ...
        sentencer = BertTokenizer(vocab_file=args.sentencer_path, tokenize_chinese_chars=False, do_lower_case=False)
    ...
    if args.enable_beat:
        ...
        beater = BertTokenizer(vocab_file=args.beater_path, tokenize_chinese_chars=False, do_lower_case=False)
    ...
    if args.enable_relative_pos:
        ...
        poser = BertTokenizer(vocab_file=args.poser_path, tokenize_chinese_chars=False, do_lower_case=False)
    ...
    for _ in range(args.nsamples):
        ...
        for out in outs:
            ...
            for i, item in enumerate(text):
                if item == '[MASK]' or item == '[SKIP]' or item == '[BEAT]':
                    text[i] = ''
                elif item == '[CLS]':
                    text[i] = '\n\n'
                elif item == '[SEP]':
...
if __name__ == '__main__':
    from module import GPT2LMHeadModel
    model = GPT2LMHeadModel.from_pretrained('model/deeprapper-model')
    print(main("我长大的地方像一个简朴的寨", model))
