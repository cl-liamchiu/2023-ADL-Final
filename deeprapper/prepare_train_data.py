...
def build_files_separate(num_pieces,
    ...
    for k in range(num_pieces):
        ...
        for i in range(len(lines)):
            line = lines[i]
            # print('line: ', line)
            cleaned_text = re.sub(r'\[BEAT\]|\[SEP\]', '', line)
...
def parse_lyric(l_content_path, l_finals_path, with_beat=False, beat_mode=0):
    ...
    if len_lyric != len_final or len_lyric != len_sentence:
        print("len_lyric != len_final != len_sentence")
        print("invalid song: ", l_content_path)
        valid = False
...
if __name__ == '__main__':
    ...
    read_lyrics(path='data/lyrics/lyrics', 
                out_content_path='data/lyrics/lyrics_samples/train/content',
                out_finals_path='data/lyrics/lyrics_samples/train/finals',
                out_sentences_path='data/lyrics/lyrics_samples/train/sentences',
                out_pos_path='data/lyrics/lyrics_samples/train/pos',
                out_beats_path='data/lyrics/lyrics_samples/train/beats',
                reverse=True, 
                with_beat=False,
                beat_mode=0)